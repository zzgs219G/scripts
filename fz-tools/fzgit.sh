...
# ── 版本号（v5.100 修复：p 以「目标远程」而非 @{u} 判断是否推送）──
FZ_VERSION="5.100"
...
\033[1;33m── 🌱 新手起步 ───────────────────────────\033[0m
  \033[36minit\033[0m       空文件夹一键初始化（自动引导绑远程/首次推送）
  \033[36msetup\033[0m      环境配置向导（git 用户名/邮箱/凭据检测）
...
\033[1;33m── 🚀 推送 & 发布 ───────────────────────\033[0m
  ...
  \033[36mp 远程名\033[0m   推送到指定远程（如 p cnb / p gitee）
  \033[36mp skip\033[0m     可跳过CI构建
  \033[36mp \"备注\"\033[0m   指定备注推送
  \033[36mp cnb skip\033[0m 可自由组合（远程+跳过CI+备注）
  \033[36mremote\033[0m     绑定远程（GitHub/GitLab/Gitee/CNB/CODING）
